...
SUBROUTINE qepy_electrons_scf ( printout, exxen )
  ...
  !
  add_descf = .FALSE.
  if ( embed%ldescf .and. embed%iterative .and. (.not. embed%initial) .and. embed%mix_coef<0.0_DP ) add_descf = .TRUE.
  !
...
100     if ( embed%iterative ) then
        ...
        if ( iter==1 .and. (embed%mix_coef>0.0_DP)) then
           if ( abs(mixing_beta_new-mixing_beta_prev)>1E-6) then
              ! the mixing difference, so restart the mixing
              CALL close_mix_file( iunmix, 'delete' )
              CALL open_mix_file( iunmix, 'mix', exst )
              mixing_beta_prev = mixing_beta_new
           else
