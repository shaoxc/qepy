...
CONTAINS
  !
  !qepy --> args
  SUBROUTINE get_command_line ( input_command_line_)
     IMPLICIT NONE
     CHARACTER(LEN=*), OPTIONAL :: input_command_line_
  !qepy <-- args
     INTEGER :: narg
     LOGICAL :: read_string
     CHARACTER(LEN=256) :: arg 
     CHARACTER(LEN=6), EXTERNAL :: int_to_char
  !qepy --> set command_line
     CHARACTER(LEN=len(command_line)):: input_command_line
     !
     IF (command_line(1:5)=='qecmd') THEN
        input_command_line = command_line(5:len(command_line))
        command_line = ' '
        read_string = .TRUE.
     ELSE
     command_line = ' '
     read_string = PRESENT ( input_command_line_ )
     ENDIF
  !qepy <-- set command_line
     !
